...
class Depart extends Controller{
    ...
    public function index(){
        ...
        $list = db('depart') -> alias('d') -> join('zcw_depart b','b.id =  d.pid','LEFT') -> field(array('d.id','d.name','d.no','d.status',
    ...
    public function add(){
        if($this -> request -> isAjax()){
            db('depart') -> data($this -> request -> except('pname')) -> insert();
            return json(['state' => 1]);
        }else{
            return $this -> view -> fetch();
        }
    ...
    public function edit(){
        if($this -> request -> isAjax()){
            ...
            db('depart') -> where(array('id' => $id)) -> data($this -> request -> except(array('id','pname'))) -> update();
            return  json(array('state' => 1));
        }else{
            $id = $this -> request -> param('id');
            $info = db('depart') -> alias('d') -> join('zcw_depart b','b.id = d.pid','LEFT') ->
